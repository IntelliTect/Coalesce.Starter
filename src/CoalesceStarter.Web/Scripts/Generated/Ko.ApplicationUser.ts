...
module ViewModels {
	...
	export class ApplicationUser
    {
        ...
        public deleteCallbacks: { ( myself: ApplicationUser ): void; } [] = [];
        // Callbacks to call after a save.
        public saveCallbacks: { ( myself: ApplicationUser ): void; } [] = [];    
        ...
        constructor(newItem?: any, parent?: any){
            ...
            self.editUrl = ko.computed(function() {
                return areaUrl + "ApplicationUser/CreateEdit?id=" + self.applicationUserId();
            });
			...
			self.save = function(callback?: any) {
				if (!self.isLoading()) {
					if (self.validate()) {
                        ...
                        var url = areaUrl + "api/ApplicationUser/Save?includes=" + self.includes + '&dataSource=';
                        if (typeof self.dataSource === "string") url += self.dataSource;
                        else url += ListViewModels.ApplicationUserDataSources[self.dataSource];
			...
			self.load = function(id: any, callback?) {
                if (!id) {
                    id = self.applicationUserId();
                }
                if (id) {
                    self.isLoading(true);
                    intellitect.utilities.showBusy();

                    var url = areaUrl + "api/ApplicationUser/Get/" + id + '?includes=' + self.includes + '&dataSource=';
                    if (typeof self.dataSource === "string") url += self.dataSource;
                    else url += ListViewModels.ApplicationUserDataSources[self.dataSource];
                    ...
                    $.ajax({ method: "GET", url: url, xhrFields: { withCredentials: true } })
                        ...
                        .fail(function() {
                            self.isLoaded(false);
                            if (self.showFailureAlerts)
                                alert("Could not get ApplicationUser with id = " + id);
                        })
                        .always(function() {
            ...
            self.reload = function(callback) {
                self.load(self.applicationUserId(), callback);
            };
            ...
            self.deleteItem = function(callback) {
                var currentId = self.applicationUserId();
                if (currentId){
                $.ajax({ method: "POST", url: areaUrl+ "api/ApplicationUser/Delete/" + currentId, xhrFields: { withCredentials: true } })
                .done(function(data) {
                    if (data) {
                        self.errorMessage('');
            ...
            self.saveCollection = function(propertyName, childId, operation) {
                ...
                var currentId = self.applicationUserId();
                $.ajax({ method: "POST", url: areaUrl + 'api/ApplicationUser/' + method + '?id=' + currentId + '&propertyName=' + propertyName + '&childId=' + childId, xhrFields: { withCredentials: true } })
                .done(function(data) {
                    self.errorMessage('');
                    self.loadFromDto(data.object, true);
                    // The object is now saved. Call any callback.
            ...
            self.showEditor = function(){
                ...
                intellitect.utilities.showBusy();
                $.ajax({ method: "GET", url: areaUrl + 'ApplicationUser/EditorHtml', data: {simple: true}, xhrFields: { withCredentials: true } })
                .done(function(data){
                    // Add to DOM
                    intellitect.webApi.setupModal('Edit Application User', data, true, false);
                    // Data bind
